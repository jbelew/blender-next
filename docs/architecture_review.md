...
*   **Location**: [`index.ts`](../packages/core/src/index.ts#L48-L61) and [`route.ts`](../apps/storefront/src/app/api/blender/route.ts#L34-L80)
...
*   **Location**: [`setNestedPath`](../apps/storefront/src/app/admin/page.tsx#L8-L27) in the admin panel.
...
*   **Location**: [`index.ts`](../packages/react/src/index.ts#L26-L64) and [`page.tsx`](../apps/storefront/src/app/admin/page.tsx#L57-L80)
*   **The Issue**:
...
*   **Location**: [`loadCollection`](../packages/core/src/index.ts#L18-L43) and [`page.tsx`](../apps/storefront/src/app/[slug]/page.tsx#L24-L35)
...
*   **Location**: [`page.tsx`](../apps/storefront/src/app/[slug]/page.tsx#L8)
*   **The Issue**: The storefront dynamic page route forces rendering dynamically: `export const dynamic = 'force-dynamic'`.
*   **Impact**: Next.js App Router caching is disabled, and ISR is entirely bypassed. This is in direct conflict with the [**Scaling Strategy Specification**](../docs/scaling_strategy.md#L39-L66) where ISR is planned to deliver sub-millisecond cached responses.
...
*   **The Issue**: The wrapper `<DynamicPageClient>` is marked `"use client"`. Consequently, every component nested inside it—including templates ([`LandingTemplate`](../apps/storefront/src/templates/LandingTemplate/LandingTemplate.tsx) and [`BlogTemplate`](../apps/storefront/src/templates/BlogTemplate/BlogTemplate.tsx)) and individual blocks (Hero, Text, ProductGrid)—is forced to render as a client component.
...
| :--- | :--- | :--- |
| **SQLite Index Cache** | [**Scaling Strategy**](../docs/scaling_strategy.md): Build `.blender/cache.db` to paginate queries and prevent slow directory scanning. | **Unimplemented**. No database package (e.g. SQLite, Bun DB) or watcher (e.g., Chokidar) exists in workspace dependency manifests. |
| **Git Campaign Integration** | [**Git Campaign Workflows**](../docs/git_campaign_workflows.md): Automate Git branches, commits, and Pull Requests when saving/publishing page structures. | **Unimplemented**. Saving modifications writes files to disk directly. No subprocess calls or Git packages exist in the API layer. |
| **Asynchronous params Types** | [**Technical Spec**](../docs/architecture_specification.md#L258-L267): React 19 / Next.js 16 breaking change updates dynamic route typing. | **Partially Implemented**. Dynamic `params` are typed correctly as promises, but `searchParams` is omitted in page parameters. |
...
2.  **RSC-First Routing Layout**: Modify [`[slug]/page.tsx`](../apps/storefront/src/app/[slug]/page.tsx) to check for Draft Mode / Search Params. If editing is inactive, render a server-only layout that streams static ISR code. Switch to `<DynamicPageClient>` only when previewing.
